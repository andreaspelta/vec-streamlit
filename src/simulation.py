...
def sample_hh(hh_params, hours_idx, S, rng):
    """Return dict of HH loads by household_id (from mapping). For demo, synthesize 3 HH."""
    # In a full system, you'd map each uploaded meter -> household_id. Here we synthesize three.
...
def sample_pv(pv_params, kWp_map, hours_idx, S, rng):
    """Shared-weather PV generation per prosumer (kWh)."""
    Senv = pv_params["S"]  # seasons x 24 shares
    ll = pv_params["loglogistic"]
    mark = pv_params["markov"]
    prosumers = sorted(kWp_map.keys())
    # Day-state chain per season: build state for each day, then per-hour shaping
    dates = pd.DatetimeIndex(hours_idx).normalize().unique()
    seasons = [season_of(pd.Timestamp(d)) for d in dates]
    # For each day, sample state via Markov with stationary init (approx)
    states = []
    prev = {}
    for d, s in zip(dates, seasons):
        ...
        if s not in prev:
            ...
            p = P[prev[s], :]
            state = int(rng.uniform() < p[1])
        prev[s] = state
        states.append((d, s, state))
    day_state = pd.DataFrame(states, columns=["date","season","state"])

    # Daily multiplier M ~ loglogistic with median ≈ 1
    Ms = []
    for d, s, _ in states:
        pars = ll[s]
        c = pars["c"]; sc = pars["scale"]
    ...
    for pid in prosumers:
        ...
        for ts in hours_idx:
            s = season_of(ts)
            h = ts.hour
            S_h = float(Senv.loc[s, h]) if (s in Senv.index and h in Senv.columns) else 0.0
            M = float(Mser.get(ts.normalize(), 1.0))
            # Beta clearness — using same alpha,beta for simplicity
            a = float(mark[s]["beta"]["alpha"]); b = float(mark[s]["beta"]["beta"])
            c = rng.beta(a, b)
            # residual as mild lognormal
            eps = rng.normal(scale=0.1)
            base = kWp * S_h * M * (c/(1+c))  # rescaled to (0,1)
            kwh.append(max(0.0, base*np.exp(eps)))
        out[pid] = np.array(kwh)
    return out
...
def run_mc(hh_params, shop_params, pv_params, prices, retail_setup, mapping, kWp_map, scenario):
    ...
    # Sample paths per scenario, then aggregate to distributions
    S = scenario["S"]
    # Minimal set of IDs (demo): from mapping and kWp_map
    prosumers = sorted(kWp_map.keys())
    households = sorted(mapping["household_id"].unique())
    shops = ["K101","K102","K103"]  # demo placeholders; extend by uploading shop mapping later

    # Zonal price per zone per hour (€/MWh -> €/kWh)
    zonal = prices["zonal"].copy()
    zonal["eur_kwh"] = zonal["zonal_price (EUR_per_MWh)"] / 1000.0

    # Per-hour flows aggregated to community level
    # For brevity we compute only community-level distributions here.
    matched_hh_all = []
    matched_shop_all = []
    import_hh_all = []
    import_shop_all = []
    ...
    for s in range(S):
        ...
        # Initialize accumulators per hour
        ...
        for t_idx, ts in enumerate(hours):
            # Prices for this hour: use zone of mapping rows; pool by zone for shops (demo: first zone)
            # Community-level: use a representative zone (first zone present)
            zones_this_hour = zonal[zonal["timestamp"] == ts.strftime("%Y-%m-%d %H:%M:%S%z")]
            if zones_this_hour.empty:
                continue
            # We'll just take the first zone row as "anchor" for demo community-level pricing:
            z_eur_kwh = zones_this_hour.iloc[0]["eur_kwh"]
            ...
            # Build vectors for HH demand at t, SHOP demand at t, prosumer surplus at t
            d_hh = np.array([hh_series[h][t_idx] for h in households])  # kWh
            d_shop = np.array([shop_series[k][t_idx] for k in shops])    # kWh
            g_pv = np.array([pv_series[p][t_idx] for p in prosumers])    # kWh (no prosumer own load)

            # STEP 1: designated HH fill (per prosumer)
            for i, pid in enumerate(prosumers):
                # HH attached to this prosumer
                ...
                # Prosumer revenue on HH matched (HH-Gift may zero this)
                pros_rev += matched.sum() * Ppros_HH
                # Consumer cost on HH matched
                cons_cost += matched.sum() * Pcons_HH
                # Platform margin on HH matched (after losses)
                platform_margin += (matched.sum() * (1-loss_factor)) * gap_HH

            # STEP 2: pool residual to shops (zone pooling simplified)
            pool = g_pv.sum()
            need_shop = d_shop.copy()
            matched_s, resid_pool = equal_level_fill(pool, need_shop)
            m_shop[t_idx] += matched_s.sum()
            d_shop -= matched_s
            g_pv_residual = resid_pool

            # Attribution to prosumers for SHOP matched (by contribution share)
            # For community totals, we only need sum, not per-prosumer splits.

            # Prosumer revenue on SHOP matched
            pros_rev += matched_s.sum() * Ppros_SH
            # Consumer cost on SHOP matched
            cons_cost += matched_s.sum() * Pcons_SH
            # Platform margin on SHOP matched (after losses)
            platform_margin += (matched_s.sum() * (1-loss_factor)) * gap_SH

            # STEP 3: Imports/Exports
            imp_hh[t_idx] += d_hh.sum()
            imp_shop[t_idx] += d_shop.sum()
            # Consumers pay retail on imports
            cons_cost += d_hh.sum() * retail_HH
            cons_cost += d_shop.sum() * retail_SH
            # Exports from residual PV
            exp_total[t_idx] += g_pv_residual
            pros_rev += g_pv_residual * delta_unm

        # Monthly fees and platform cost (approx: 12 months)
        pros_rev -= 12.0 * 0.0  # no per-prosumer fee modeled here for brevity
        cons_cost += 12.0 * (scenario["fee_hh"] + scenario["fee_shop"])  # simplistic
        platform_margin += 12.0 * (scenario["fee_pros"] + scenario["fee_hh"] + scenario["fee_shop"]) - scenario["platform_monthly"]

        matched_hh_all.append(m_hh.sum())
        matched_shop_all.append(m_shop.sum())
        import_hh_all.append(imp_hh.sum())
        import_shop_all.append(imp_shop.sum())
...
def sankey_aggregate(mc):
    # Simple community-level Sankey
    total_gen = mc["matched_hh"].mean() + mc["matched_shop"].mean() + mc["export"].mean()
    labels = ["PV Gen", "HH matched", "SHOP matched", "Export"]
    source = [0, 0, 0]
    target = [1, 2, 3]
    value = [mc["matched_hh"].mean(), mc["matched_shop"].mean(), mc["export"].mean()]
...
def price_panel_example(mc, scenario):
    # Just a stylized bar to illustrate HH vs SHOP price components on a representative hour
    spread_hh = scenario["alpha_hh"], scenario["phi_hh"]
    spread_shop = scenario["alpha_shop"], scenario["phi_shop"]
    fig = go.Figure()
